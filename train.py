...
def parse_args():
    ...
    parser.add_argument('--name', default='Solar', help='model name: (default: arch+timestamp)')
    parser.add_argument('--epochs', default=400, type=int, metavar='N', help='number of total epochs to run')
    parser.add_argument('-b', '--batch_size', default=32, type=int, metavar='N', help='mini-batch size (default: 16)')
    ...
    # dataset
    parser.add_argument('--dataset', default='PVEL_S', help='dataset name')      
    parser.add_argument('--data_dir', default='/home/lm/project/data/solar/', help='dataset dir')
    ...
    parser.add_argument('--kan_lr', default=1e-2, type=float, metavar='LR', help='initial learning rate')
    parser.add_argument('--kan_weight_decay', default=1e-4, type=float, help='weight decay')
    parser.add_argument('--scheduler', default='MultiStepLR', choices=['CosineAnnealingLR', 'ReduceLROnPlateau', 'MultiStepLR', 'ConstantLR'])
...
def indicators1(output, target):
    ...
    return iou_, dice_, iou_, dice_, recall_, specificity_, precision_
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target, meta in val_loader:
            ...
            if config['deep_supervision']:
                ...
                loss /= len(outputs)
                iou, dice, _ = iou_score(outputs[-1], target)
                iou_, dice_, _, _, recall_, specificity_, precision_ = indicators1(outputs[-1], target)
            else:
                output = model(input)
                loss = criterion(output, target)
                iou, dice, _ = iou_score(output, target)
                iou_, dice_, _, _, recall_, specificity_, precision_ = indicators1(output, target)

            avg_meters['loss'].update(loss.item(), input.size(0))
            avg_meters['iou'].update(iou, input.size(0))
            avg_meters['dice'].update(dice, input.size(0))
            avg_meters['sp'].update(specificity_, input.size(0))
            avg_meters['pr'].update(precision_, input.size(0))
            avg_meters['rc'].update(recall_, input.size(0))
    ...
    return OrderedDict([('loss', avg_meters['loss'].avg),
                        ...
                        ('pr', avg_meters['pr'].avg),
                        ('rc', avg_meters['rc'].avg)])
...
def main():
    ...
    if dataset_name == 'busi':
        mask_ext = '_mask.png'
    elif dataset_name == 'PVEL_S':
        mask_ext = '_mask.jpg'

    # Data loading code
    train_img_ids = sorted(glob(os.path.join(config['data_dir'], config['dataset'], 'train_test', 'defect', '*' + img_ext)))
    train_img_ids = [os.path.splitext(os.path.basename(p))[0] for p in train_img_ids] 
    val_img_ids = sorted(glob(os.path.join(config['data_dir'], config['dataset'], 'test', 'defect', '*' + img_ext)))
    ...
    train_dataset = Dataset(
        ...
        img_dir=os.path.join(config['data_dir'], config['dataset'], 'train_test/defect'),
        mask_dir=os.path.join(config['data_dir'], config['dataset'], 'train_test/masks'),
        img_ext=img_ext,
        mask_ext=mask_ext,
    ...
    val_dataset = Dataset(
        img_ids=val_img_ids,
        img_dir=os.path.join(config['data_dir'] ,config['dataset'], 'test/defect'),
